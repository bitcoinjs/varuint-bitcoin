...
export function encode (n: number, buffer: Buffer, offset: number): {buffer: Buffer, bytes: number} {
  checkUInt53(n)
...
export function decode (buffer: Buffer, offset: number): {value: number, bytes: number} {
  if (!Buffer.isBuffer(buffer)) throw new TypeError('buffer must be a Buffer instance')
  if (!offset) offset = 0
...
export function encodingLength (n: number) {
  checkUInt53(n)
